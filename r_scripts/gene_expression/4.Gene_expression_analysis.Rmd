...
summary(weibull_)
```
Write a function to apply alice's package on each gene to test for hybrid differences
```{r}

parasite_load_genes <- function(x) {
  field$Sex <- as.factor(field$Sex)

speparam <- parasiteLoad::getParamBounds("weibull", data = field, response = toString(x))


##All
print(parasiteLoad::analyse(data = field,
                        response = toString(x),
                        model = "weibull",
                        group = "Sex"))
...
```{r include = FALSE}
lapply(Genes_field, parasite_load_genes)
```
