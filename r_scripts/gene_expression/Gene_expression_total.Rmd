...
g <- gene_na_omit %>% select(c("CXCR3_bio", "IL.6",
  "IL.10", "IL.13", "IL.10", "IL.13", "IL1RN", "CASP1", "CXCL9", 
  "IDO1", "IRGM1", "MPO", "MUC2", "MUC5AC", "MYD88", 
  "NCR1", "PRF1", "RETNLB", "SOCS1", "TICAM1", "TNF")) %>% na.omit()

g2 <- gene_na_omit %>% select(-c(max_WL, delta))
...
#number of pca dimensions, to use when imputing mising 
nb <- estim_ncpPCA(g3, scale = TRUE) #estimate the number of dimensions
comp <- imputePCA(g3, ncp = 2) #impute the table
