...
### Data tidying for the heatmap function
immune <- immune_data  %>% 
  dplyr::select(-c(primary_infection, challenge_infection, infection_history, 
                   mouse_strain, max_WL, delta, Parasite_challenge, F_Treg_prop, F_IL17A_CD4, F_delta))
...
# turn the first row into column names
immune %>%
  row_to_names(row_number = 1) -> immune

# Now further prepare the data frame for plotting by removing the first row
## and convert the column to row names with the cells 
immune[-1, ] -> heatmap_data
...
# Match the row names to the heatmap data frame
rownames(annotation_df) <- colnames(immune)
