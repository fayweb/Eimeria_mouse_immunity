...
gene_imp <- gene_expr %>% left_join(imputed_gene, by = c("pc1", "pc2"))
...
rm(basics_gene, circ, comp, g, gene_correlation, mi, nb, 
...
facs_data <- Challenge_facs %>% 
   dplyr::select(c(EH_ID, primary_infection, challenge_infection, 
                   infection_history, mouse_strain, max_WL, delta)) %>%
     inner_join((t(heatmap_data) %>% 
                   as.data.frame() 
                 %>% tibble::rownames_to_column("EH_ID")), 
                by = "EH_ID")
...
## 2. Imputation of the missing values using the function impute PCA
comp <- imputePCA(f[12:26], ncp = 2, scale = TRUE) #impute the table
head(comp$completeObs) # resulting imputed data table
# this estimate should be better than simply using the mean, beacause we've taken into account
# the links between variables and similarities between individuals in its calculation
...
mi <- MIPCA(f[12:26], scale = TRUE, ncp = 2)
plot(mi)
```
Individuals lying on the axis have no missing data, but individuals that far away have many missing data. 
big ellipse = big uncertainty
...
fviz_pca_ind(res.pca,
             geom.ind = "point", # show points only (nbut not "text")
             col.ind = f$Parasite_challenge, # color by groups
             palette = c("#00AFBB", "#E7B800", "#FC4E07"),
             addEllipses = TRUE, # Concentration ellipses
             legend.title = "Groups"
             )
...
# Add confidence ellipses
fviz_pca_ind(res.pca, geom.ind = "point", col.ind = f$Parasite_challenge, 
             palette = c("#00AFBB", "#E7B800", "#FC4E07"),
             addEllipses = TRUE, ellipse.type = "confidence",
             legend.title = "Groups"
             )
...
```{r, echo = FALSE}
fviz_pca_biplot(res.pca, 
                col.ind = f$Parasite_challenge, palette = "jco", 
                addEllipses = TRUE, label = "var",
                col.var = "black", repel = TRUE,
                legend.title = "Parasite in the challenge infection") 
```
...
fviz_pca_biplot(res.pca, 
                # Individuals
                geom.ind = "point",
                fill.ind = f$Parasite_challenge, col.ind = "black",
                pointshape = 21, pointsize = 2,
                palette = "jco",
                addEllipses = TRUE,
                # Variables
...
```{r, echo = FALSE}
weight_lm <- lm(max_WL ~ pc1 + pc2 + Parasite_challenge, data = f)

summary(weight_lm)

AIC(weight_lm)

weight_lm_exp_only <- lm(max_WL ~ pc1 + pc2, data = f)
summary(weight_lm_exp_only)
...
immune <- immune_data  %>% 
  dplyr::select(-c(EH_ID, primary_infection, challenge_infection, infection_history, 
                   mouse_strain, max_WL, delta, Parasite_challenge, F_Treg_prop, F_IL17A_CD4, F_delta))
...
#because of the amount of nas in some columns, no correlations can be calculated
#therefore I will have to remove these two columns: Treg_prop, IL17A_CD4
...
corrplot(immune_correlation, 
         ...
         p.mat = p.mat, sig.level = 0.01, insig = "blank",
         addCoef.col = 'black',#Add significance level to the correlogram
         number.cex=0.3) #coefficients size
#remove the values that are insignificant
...
# this estimate should be better than simply using the mean, beacause we've taken into account
# the links between variables and similarities between individuals in its calculation
imputed <- as.data.frame(comp$completeObs)
...
imputed$pc1 <- res.pca$ind$coord[, 1]
imputed$pc2 <- res.pca$ind$coord[, 2] 
#We also need to extract the data for the variable contributions to each of the pc axes.
pca.vars <- res.pca$var$coord %>% data.frame
pca.vars$vars <- rownames(pca.vars)
pca.vars.m <- melt(pca.vars, id.vars = "vars")
...
#Now we can make our initial plot of the PCA.
i <- immune_data %>%
  dplyr::select(-c(Parasite_challenge, F_delta)) 


i %>%
  pivot_longer(cols = 8:41, names_to = "Immune_parameter", values_to = "measurement")  %>%
  ggplot(aes(x = pc1, y = pc2, color = challenge_infection, shape = challenge_infection)) +
  geom_hline(yintercept = 0, lty = 2) +
  geom_vline(xintercept = 0, lty = 2) +
  geom_point(alpha = 0.8) +
...
```{r, echo = FALSE}
weight_lm <- lm(max_WL ~ pc1 + pc2 + challenge_infection, data = i)

summary(weight_lm)

AIC(weight_lm)

weight_lm_exp_only <- lm(max_WL ~ pc1 + pc2, data = i)
summary(weight_lm_exp_only)
...
```{r, echo = FALSE}
### Prepare the annotation data frame for the heatmap
annotation_df <- immune_data %>%
  dplyr::select(c("EH_ID", "Parasite_challenge", "infection_history"))

### Data tidying for the heatmap function
immune <- immune_data %>% left_join(imputed, by = c("pc1", "pc2"))

#remove all columns of the non-imputed data
immune = immune[,!grepl(".x$",names(immune))]

#remove the suffix y
immune <- immune %>% rename_with(~str_remove(., '.y'))
