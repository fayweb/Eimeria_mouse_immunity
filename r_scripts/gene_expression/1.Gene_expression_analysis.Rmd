...
# we want to #1 Normalize Ct (target gene) to Ct reference gene
# to do that we need the calibrator genes: 
# Dct calibrator= Ct(target of calibrator) - Ct(reference of calibrator)
cal <- function(x) {x - gu$PPIB} # function of calibrator

gu <- gu %>% 
  dplyr::mutate(
    IFNy_cal = cal(IFNy),
    CXCR3_bio_cal  = cal(CXCR3_bio),
    IL.6_cal = cal(IL.6),
    IL.10_cal = cal(IL.10),
    IL.13_cal = cal(IL.13),
    IL1RN_cal = cal(IL1RN),
    ...
    MPO_cal  = cal(MPO),
    MUC2_cal = cal(MUC2),
    MUC5AC_cal = cal(MUC5AC),
    MYD88_cal  = cal(MYD88),
    NCR1_cal  = cal(NCR1),
    PRF1_cal = cal(PRF1),
    RETNLB_cal = cal(RETNLB),
    SOCS1_cal = cal(SOCS1),
    TICAM1_cal  = cal(TICAM1),
    TNF_cal= cal(TNF))
...
gene <- gene %>%
  ...
  #delta ct of calibrator - delta ct test sample
  dplyr::mutate(
    IFNy_N = 2^-((IFNy - PPIB) - gu$IFNy_cal),
    CXCR3_bio_N  = 2^-((CXCR3_bio - PPIB)- gu$CXCR3_bio_cal),
    IL.6_N = 2^-((IL.6 - PPIB)- gu$IL.6_cal),
    IL.10_N = 2^-((IL.10 - PPIB)- gu$ IL.10_cal),
    IL.13_N = 2^-((IL.13 - PPIB)- gu$IL.13_cal),
    IL1RN_N = 2^-((IL1RN - PPIB)- gu$IL1RN_cal),
    CASP1_N = 2^-((CASP1 - PPIB)- gu$CASP1_cal),
    CXCL9_N  = 2^-((CXCL9 - PPIB)- gu$CXCL9_cal),
    IDO1_N = 2^-((IDO1 - PPIB)- gu$IDO1_cal),
    IRGM1_N = 2^-(( IRGM1 - PPIB)- gu$IRGM1_cal),
    MPO_N  =  2^-((MPO - PPIB) - gu$MPO_cal),
    MUC2_N = 2^-((MUC2 - PPIB)- gu$MUC2_cal),
    MUC5AC_N = 2^-((MUC5AC - PPIB)- gu$MUC5AC_cal),
    MYD88_N  = 2^-((MYD88 - PPIB)- gu$MYD88_cal),
    NCR1_N  = 2^-((NCR1 - PPIB)- gu$NCR1_cal),
    PRF1_N = 2^-((PRF1 - PPIB)- gu$PRF1_cal),
    RETNLB_N =  2^-((RETNLB - PPIB)- gu$RETNLB_cal),
    SOCS1_N = 2^-((SOCS1 - PPIB)- gu$SOCS1_cal),
    TICAM1_N  = 2^-((TICAM1 - PPIB)- gu$TICAM1_cal),
    TNF_N = 2^-((TNF - PPIB) - gu$TNF_cal)
  )
