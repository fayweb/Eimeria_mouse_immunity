...
#add this to the bid data frame 

Field %>% 
  dplyr::select(all_of(Gene.Exp.cols)) %>%
  [rowSums(is.na(Field %>% 
  dplyr::select(all_of(Gene.Exp.cols)))) != ncol(Field %>% 
  dplyr::select(all_of(Gene.Exp.cols))), ]
  

Field[ , Gene.Exp.cols] <- [rowSums(is.na(Field[ , Gene.Exp.cols])) != ncol(Field[ , Gene.Exp.cols]), ]
