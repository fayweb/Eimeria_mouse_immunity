...
# of the wild mice 
#g <- g %>% 
 # dplyr::mutate(wen = (weight / weight_dpi0) * 100)
...
result   %>%
  ggplot() +
  geom_point(aes(x = `result$prediction`, y = max_WL, color = Parasite_challenge)) 
```
```{r}
cor(x = result$`result$prediction`, y = result$max_WL)
```
