...
result_parasite   %>%
  ggplot() +
  geom_point(aes(x = predictions_parasite, y = current_infection, color = current_infection)) +
  geom_abline() +
  labs(x = "Predictions: current infection ", y = "Observed: current infection",
         title = "Predicting current infection in response to immune gene expression") +
    theme_bw()
...
result_parasite   %>%
  ggplot() +
  geom_point(aes(x = predictions_parasite, y = Parasite_challenge, color = Parasite_challenge)) +
  geom_abline() +
  labs(x = "Predictions: current infection ", y = "Observed: current infection",
         title = "Predicting current infection in response to immune gene expression") +
    theme_bw()
...
genes <- genes[rowSums(is.na(genes)) == 0, ]

 r <- Filter(function(x)!all(is.na(x)), Field[ , all_of(Gene.Exp.cols)]) 

#genes <- Field[ , all_of(Gene.Exp.cols)]

 # remove columns with only NAs 
# genes <- Filter(function(x)!all(is.na(x)), genes) 
 
# filter all columns with only nas
#genes <- genes[, colSums(is.na(genes)) != nrow(genes)]


#r <- filter(Field, rowSums((Field[ , all_of(Gene.Exp.cols)]) != "NA")
```

#### Ditch the previous step, let's now try a weird imputation of all the data

```{r impute_field}
#Start by selecting only the genes 
...
# iter = how many random forests are needed, in theory 6 are enough
f.imputed <- rfImpute(max_WL ~ ., data = g.1, iter = 6)
g.imputed <- g.imputed %>% dplyr::select(-max_WL)
g <- g %>% 
  dplyr::select(-all_of(Genes)) 
#full data set containing the imputed gene expression data
