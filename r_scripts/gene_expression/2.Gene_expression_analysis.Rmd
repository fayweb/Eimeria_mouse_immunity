...
g.1 <- g %>%
  dplyr::select(c(end_rel_weight, all_of(Genes)))
...
g.imputed <- rfImpute(end_rel_weight ~ ., data = g.1, iter = 6)

g.imputed <- g.imputed %>% dplyr::select(-end_rel_weight)
...
training.samples <- g.imputed$end_rel_weight%>%
...
```{r random_forest}
#train the model
model <- randomForest(end_rel_weight ~., data = train.data, proximity = TRUE)
...
result   %>%
  ggplot() +
  geom_point(aes(x = `result$prediction`, y = end_rel_weight, color = Parasite_challenge)) +
  geom_abline() 
  
```
```{r}
cor(x = result$`result$prediction`, y = result$end_rel_weight)
```


```{r}
plot(y =result$end_rel_weight, x = result$`result$prediction`, 
