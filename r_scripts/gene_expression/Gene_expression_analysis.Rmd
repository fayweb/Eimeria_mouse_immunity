...
basics_gene <- as_tibble(Challenge) %>%
  ...
  dplyr::select(c("EH_ID", "primary_infection", "challenge_infection", 
                  "Parasite_challenge", "infection_history","mouse_strain", 
                  "max_WL", "delta", "hybrid_status", all_of(Genes)))
...
## First select the common mice between your two data frames
 ...
 gene_expr <- basics_gene %>% 
   dplyr::select(c(EH_ID, primary_infection, challenge_infection, Parasite_challenge,
                   infection_history, mouse_strain, max_WL, delta, hybrid_status)) %>%
...
gene_expr_s <- gene_expr %>%
    pivot_longer(cols = 10:29, names_to = "Gene", values_to = "gene_expression")
...
```{r histo_genes_parasites, echo = FALSE, message =FALSE}
gene_expr %>%
    group_by(EH_ID) %>%
...
imputed_expr <- g %>% left_join(imputed_gene, by = c("pc1", "pc2"), 
                                suffix = c("_x", "_y"))
...
write.csv(gene_expr, "output_data/gene_expression/data_products/clean_gene_expression.csv", row.names = FALSE)
