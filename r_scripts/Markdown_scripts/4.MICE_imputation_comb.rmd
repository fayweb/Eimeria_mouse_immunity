...
count_data <- hm_selection_g %>%
  dplyr::select(c(Mouse_ID, all_of(Gene_lab), "GAPDH", "PPIB"))

# row names are samples names

rownames(count_data) <- make.names(count_data[, 1], unique = TRUE)

count_data <- count_data %>% dplyr::select(-Mouse_ID)

count_data <- as.matrix(unique(count_data))


count_data <- as.data.frame(t(count_data))
#count_field$Mouse_ID <- str_remove(count_field$Mouse_ID, "AA_")




meta_data <- hm_selection_g %>%
...
meta_data$origin <- as.factor(meta_data$origin)



# construct deseq data set object
dds_field <- DESeqDataSetFromMatrix(countData= count_data, 
                              colData=meta_data, 
