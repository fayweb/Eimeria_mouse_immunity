...
normalize_gene <- function(x, y) { # x = target gene, y = housekeeping gene
 dct <- count_field$x - count_field$y

 dct_mean <- mean(count_field$dct, na.rm = TRUE)
...
x <- normalize_gene(IL.13, GAPDH) 
  
normalize_gene <- function(x, y) { 
   count_field <- count_field %>%
  mutate(dct = y - x) %>%
  mutate(dct_mean = mean(count_field$dct_mean, na.rm = TRUE)) %>%
  # fold gene expression = 2^-DDCt
  mutate(N = 2^ - (dct - dct_mean)) %>%
  mutate(N = round(paste(x, "_N", sep = ""), digits = 2)) 
}
...
#%>%
  # fold gene expression = 2^-DDCt
  mutate(IL.13_N = 2^ - (IL.13_dct - IL.13_dct_mean))
