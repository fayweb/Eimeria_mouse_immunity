...
gene <- lab %>%
  dplyr::select(c(Mouse_ID, max_WL)) %>%
...
test_lab <- lab %>%
  left_join(result, by = c("max_WL", "IFNy", "CXCR3", "IL.6", "IL.13", "IL.10",
                "IL1RN","CASP1", "CXCL9", "IDO1", "IRGM1", "MPO", 
                "MUC2", "MUC5AC", "MYD88", "NCR1", "PRF1", "RETNLB", "SOCS1", 
                "TICAM1", "TNF"))
...
cor(test_lab$predictions, test_lab$max_WL)

test_lab   %>%
  ggplot(aes(x = predictions, y = max_WL, color = current_infection, 
...
```{r  }
#train the model
weight_loss_predict <- randomForest(max_WL ~., data = gene, 
                                    proximity = TRUE, ntree = 1000) 
# ntree = number of trees     
# save the model 
# toa = trained on all
save(weight_loss_predict, file =  "r_scripts/models/toa_predict_weight_loss.RData")
print(weight_loss_predict)
```