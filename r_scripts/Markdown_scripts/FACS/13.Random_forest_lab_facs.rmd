...
hm_genes <- read.csv("output_data/2.imputed_MICE_data_set.csv")
```
...
lab <- lab %>% 
  left_join(genes, by = "Mouse_ID")

lab <- lab %>%
  drop_na(IFNy)

immune <- lab %>% 
  dplyr::select(c(Mouse_ID, WL_max, "IFNy", "CXCR3", "IL.6", "IL.13", #"IL.10", 
                  "IL1RN","CASP1", "CXCL9", "IDO1", "IRGM1", "MPO", 
                  "MUC2", "MUC5AC", "MYD88", "NCR1", "PRF1", "RETNLB", "SOCS1", 
                  "TICAM1", "TNF","CD4", "Treg", "Div_Treg", "Treg17", "Th1", 
                    "Div_Th1", "Th17", "Div_Th17", "CD8", "Act_CD8", 
...
training.samples <- immune$WL_max%>%
...
train.data <- immune[training.samples, ] 
test.data <- immune[-training.samples, ] 
```
...
# save the model 
save(weight_loss_predict, file =  "r_scripts/models/facs_predict_weight_loss.RData")
