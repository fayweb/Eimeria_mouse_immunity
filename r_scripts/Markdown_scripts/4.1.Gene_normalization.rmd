...
First, we normalized our target gene (TAR) expression levels to a reference gene (REF), a process known as delta cycle threshold (ΔCq) calculation. This was done to control for variations between samples that may have arisen due to differences in PCR efficiency or loading discrepancies (refer to a paper on your reference gene selection and the ΔCq method, e.g., Vandesompele et al., 2002).

After normalization to the reference gene, we then transformed the ΔCq values using an exponential function (2^(-ΔCq)), a process known as exponential expression transform. This transformation allowed us to convert the values into relative expression levels, providing a clear representation of gene expression fold changes (Livak and Schmittgen, 2001).
...
Finally, we calculated the percentage of knockdown (% KD) using the relative expression values (ΔΔCq). The % KD quantifies the extent of gene knockdown or inhibition (refer to a paper explaining the concept and calculation of % KD).
...
Step 1. Normalize to (REF): DCq = Cq (TAR) – Cq (REF)
Step 2. Exponential expression transform: DCq Expression = 2–DCq
Step 3. Average replicates and calculate standard deviation
Step 4. Normalize to treatment control
Step 5. % KD = (1 – DDCq ) × 100
