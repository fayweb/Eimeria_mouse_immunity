...
hm$infection <- as.factor(hm$infection)
...
test_lab   %>%
  ggplot(aes(x = predictions, y = WL_max, color = infection, 
                 size = delta_ct_cewe_MminusE)) +
  geom_smooth(method = lm, se = FALSE) +
  labs(x = "Predictions: Maximum weight loss", 
       y = "Observed: Maximum weight loss") +
...
```{r }
lab$infection <- as.character(lab$infection)
lab2 <- lab %>% 
  dplyr::filter(infection %in% c("E_ferrisi", "uninfected")) %>%
    dplyr::filter(!infection == "E_falciformis")
