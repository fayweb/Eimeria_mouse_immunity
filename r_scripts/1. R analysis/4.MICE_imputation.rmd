...
lab[rowSums(is.na(lab)) != ncol(lab), ]
...
pattern_na <-as.data.frame(md.pattern(lab))
...
igf <- mice(lab, m = 5, seed = 500) # method = meth,
summary(igf)
...
complete_lab <- complete(igf, 1)

#visualize missingness
vis_miss(complete_lab)

#sapply(complete_lab, function(x) sum(is.na(x)))

imp_lab <- complete_lab %>%
  dplyr::select("CD4", "Treg", "Div_Treg", "Treg17", "Th1", 
                    "Div_Th1", "Th17", "Div_Th17", "CD8", "Act_CD8", 
                    "Div_Act_CD8", "IFNy_CD4", "IFNy_CD8",
                    "IL17A_CD4", all_of(Gene_lab))
...
# m=5 refers to the number of imputed datasets. Five is the default value.
igf <- mice(field, m = 5, seed = 500) # method = meth,
summary(igf)
