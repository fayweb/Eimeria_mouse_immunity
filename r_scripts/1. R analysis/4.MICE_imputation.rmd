...
igf <- mice(lab_sel[, -1], m = 5, seed = 500) # method = meth,
summary(igf)
...
colnames(imp_lab) <- paste(colnames(imp_lab), "imp", sep = "_")

#now join it to the full data set of the laboratory infections
lab <- cbind(lab, imp_lab)
...
imputed_hm <- full_join(field, lab, 
                  by = intersect(colnames(field), colnames(lab)))
