...
colnames(lab_genes) <- paste(colnames(lab_genes), "imp", sep = "_")


lab_genes$pc1 <- res.pca$ind$coord[, 1] # indexing the first column

lab_genes$pc2 <- res.pca$ind$coord[, 2]  # indexing the second column


#join to the data frame to have the data together
lab_genes <- hm %>%
  dplyr::filter(origin == "Lab") %>%
  left_join(lab_genes, by = intersect(colnames(lab_genes), colnames(hm)))
...
fviz_pca_biplot(res.pca, 
                col.ind = lab_genes$Parasite_challenge, palette = "jco", 
...
weight_lm <- lm(max_WL ~ pc1 + pc2 + Parasite_challenge, data = lab_genes)
summary(weight_lm)
AIC(weight_lm)
```
...
weight_lm2 <- lm(max_WL ~ pc1 + pc2 + Parasite_challenge + hybrid_status, data = lab_genes)
summary(weight_lm2)
AIC(weight_lm2)
```
...
weight_lm3 <- lm(max_WL ~ pc1 + pc2 + hybrid_status, data = lab_genes)
weight_no_pc1 <- lm(max_WL ~ pc2 + hybrid_status, data = lab_genes)
weight_no_pc2 <- lm(max_WL ~ pc1  + hybrid_status, data = lab_genes)
weight_no_hybrid <- lm(max_WL ~ pc1 + pc2, data = lab_genes)
...
weight_lm4 <- lm(max_WL ~ pc1 + pc2 + infection_history, data = lab_genes)
summary(weight_lm4)
AIC(weight_lm4)
...
```{r lm_pc, echo = FALSE}
weight_lm_exp_only <- lm(max_WL ~ pc1 + pc2, data = lab_genes)
...
gene <- lab_genes %>% dplyr::select(c(Mouse_ID, all_of(Gene_lab)))
...
annotation_df <- as_tibble(lab_genes) %>%
...
gene <- lab_genes %>% dplyr::select(c(Mouse_ID, c("IFNy", "IL.13", "PRF1", 
                                                  "TICAM1")))
...
annotation_df <- as_tibble(Challenge) %>%
  ...
  dplyr::select(c("Mouse_ID", "Parasite_challenge",
                  "hybrid_status")) %>%
